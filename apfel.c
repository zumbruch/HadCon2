...
#define APFEL_writePort(val,A,pinSetIndex,sideSelection) \
		{PORT##A = (pinSetIndex-1)? \
				((~APFEL_PIN_MASK_DIN) & ((PIN##A & ~(APFEL_PIN_MASK2)) | ((val | sideSelection?0x1 << APFEL_PIN_SS2:0 ) & APFEL_PIN_MASK2))): \
				((~APFEL_PIN_MASK_DIN) & ((PIN##A & ~(APFEL_PIN_MASK1)) | ((val | sideSelection?0x1 << APFEL_PIN_SS1:0 ) & APFEL_PIN_MASK1))); \
...
#define APFEL_readPort(A,pinSetIndex) ((PIN##A >> (APFEL_PIN_DIN##pinSetIndex )) & 0x1)
	...
	inline int8_t apfelWritePort(uint8_t val, char port, uint8_t pinSetIndex, uint8_t sideSelection)
	{
		...
		{
			case 'A':
				APFEL_writePort(val, A, pinSetIndex, sideSelection)
				;
				break;
			case 'B':
				APFEL_writePort(val, B, pinSetIndex, sideSelection)
				;
				break;
			case 'C':
				APFEL_writePort(val, C, pinSetIndex, sideSelection)
				;
				break;
			case 'D':
				APFEL_writePort(val, D, pinSetIndex, sideSelection)
				;
				break;
			case 'E':
				APFEL_writePort(val, E, pinSetIndex, sideSelection)
				;
				break;
			case 'F':
				APFEL_writePort(val, F, pinSetIndex, sideSelection)
				;
				break;
			case 'G':
				APFEL_writePort(val, G, pinSetIndex, sideSelection)
				;
				break;
			default:
				return -1;
			...
			uint16_t chipId, uint8_t quiet)
	{
		...
		snprintf_P(uart_message_string, BUFFER_SIZE - 1, PSTR("%sport/pinSet/side/dac/chipId:'%c/%x/%x/%x/%x %x"),
				uart_message_string, port, pinSetIndex, sideSelection, dacNr, chipId, value);
